"""Wrapper for a GOES-R satellite image."""
import logging
import os
...
# TODO to 2km resolution
# TODO filter bad pixels
# TODO tests
...
class GoesScan:
    ...
    def plot(self, ax=None, **imshow_kwargs):
        ...
        if self.channel in range(1, 6):
            ax.set_title(f"Reflectance Factor Band \n{title_info}", fontsize=20)
            ax.imshow(self.dataset.reflectance_factor, **imshow_kwargs)
            return ax

        ax.set_title(f"Brightness Temperature Band \n{title_info}", fontsize=20)
        ax.imshow(self.dataset.brightness_temperature, **imshow_kwargs)
        return ax
